...
public class ResourceDialog extends SpeedyFrame
{
    ...
    private List<RequestedSwapDTO> swapRequests;
    ...
    public ResourceDialog(String title, boolean resizable, boolean closable, boolean maximizable, boolean iconifiable, SpeedyView parentFrame)
    {
        ...
        new TableFilterHeader(tbSwapRequests);
        pgMain.setMinimum(0);
    ...
    private void fillPostProcessing(Long eventId)
    {
        if (eventId != null)
        {
            ...
            treeTablePostProcessingTarget.setModel(new TreeTableDataModel(
                    ResourcePlanningClientRoutines.createDataStructure(eventId, false)));
            // swap requests
            swapRequests = AppSingleton.getInstance().getPort().queryRequestedSwaps(eventId).getItem();
            tbSwapRequests.setModel(TableModelBuilder.createGenericTableModel(swapRequests));
        }
    }
    ...
    private void initComponents()
    {
        ...
        borderRequests = new JPanel();
        scSwapRequests = new JScrollPane();
        tbSwapRequests = new ResourcePlanningTable();
        btnAbortSwapRequest = new JButton();
        ...
        //======== tdbMain ========
        {
            ...
            //======== pnlPostProcessing ========
            {
                ...
                ((GridBagLayout)pnlPostProcessing.getLayout()).rowHeights = new int[] {0, 0, 0, 0, 0};
                ((GridBagLayout)pnlPostProcessing.getLayout()).columnWeights = new double[] {1.0, 1.0E-4};
                ((GridBagLayout)pnlPostProcessing.getLayout()).rowWeights = new double[] {1.0, 0.0, 1.0, 1.0, 1.0E-4};
                ...
                //======== borderRequests ========
                {
                    borderRequests.setBorder(new TitledBorder("Tauschanfragen"));
                    borderRequests.setLayout(new GridBagLayout());
                    ((GridBagLayout)borderRequests.getLayout()).columnWidths = new int[] {0, 0, 0};
                    ((GridBagLayout)borderRequests.getLayout()).rowHeights = new int[] {0, 0, 0};
                    ((GridBagLayout)borderRequests.getLayout()).columnWeights = new double[] {1.0, 0.0, 1.0E-4};
                    ((GridBagLayout)borderRequests.getLayout()).rowWeights = new double[] {0.0, 1.0, 1.0E-4};

                    //======== scSwapRequests ========
                    {
                        scSwapRequests.setViewportView(tbSwapRequests);
                    }
                    borderRequests.add(scSwapRequests, new GridBagConstraints(0, 0, 1, 2, 0.0, 0.0,
                        GridBagConstraints.CENTER, GridBagConstraints.BOTH,
                        new Insets(0, 0, 0, 5), 0, 0));

                    //---- btnAbortSwapRequest ----
                    btnAbortSwapRequest.setText("Abbrechen");
                    btnAbortSwapRequest.addActionListener(new ActionListener() {
                        @Override
                        public void actionPerformed(ActionEvent e) {
                            btnAbortSwapRequestPressed(e);
                        }
                    });
                    borderRequests.add(btnAbortSwapRequest, new GridBagConstraints(1, 0, 1, 1, 0.0, 0.0,
                        GridBagConstraints.CENTER, GridBagConstraints.BOTH,
                        new Insets(0, 0, 5, 0), 0, 0));
                }
                pnlPostProcessing.add(borderRequests, new GridBagConstraints(0, 3, 1, 1, 0.0, 0.0,
                    GridBagConstraints.CENTER, GridBagConstraints.BOTH,
                    new Insets(0, 0, 0, 0), 0, 0));
            }
    ...
    private JPanel borderRequests;
    private JScrollPane scSwapRequests;
    private ResourcePlanningTable tbSwapRequests;
    private JButton btnAbortSwapRequest;
